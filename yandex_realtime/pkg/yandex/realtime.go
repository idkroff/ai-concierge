...
func (c *Client) Connect() error {
	...
	if err := c.conn.ReadJSON(&rawEvent); err != nil {
		return fmt.Errorf("ошибка чтения события: %w", err)
	}

	// Парсим в структуру
	var created Event
	if err := json.Unmarshal(rawEvent, &created); err != nil {
		return fmt.Errorf("ошибка парсинга события: %w", err)
	}

	if created.Type == "error" {
		// Выводим сырой JSON для отладки
		return fmt.Errorf("получена ошибка от API. Полное событие: %s", string(rawEvent))
	}
